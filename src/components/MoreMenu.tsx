...
export function MoreMenu({ isCollapsed, justifyClass, hideTextClass }: Readonly<MoreMenuProps>) {
    ...
    <>
      ...
      >
        <DropdownMenuTrigger asChild>
          <button className={`w-full flex items-center ${justifyClass} p-3 rounded-full hover:bg-gray-100 transition-all outline-none group ${isCollapsed ? 'justify-center' : 'px-4 gap-3'}`}>
            <MenuIcon size={24} className="text-gray-600 group-hover:scale-105 transition-transform" />
          ...
          {menuView === 'main' && (
            <div className="space-y-1 animate-in slide-in-from-left-2 duration-200">
                <DropdownMenuItem asChild>
                    <Link to="/settings" className="p-3 cursor-pointer rounded-lg hover:bg-gray-100 focus:bg-gray-100 flex items-center gap-3 group">
                        <Settings size={20} className="text-gray-600" />
                        <span className="font-medium text-gray-700 flex-1">Settings</span>
                    </Link>
                </DropdownMenuItem>

                {/* Display Trigger: Changes state instead of opening modal */}
                <DropdownMenuItem 
                    onSelect={(e) => {
                        e.preventDefault(); // Prevent closing
                        setMenuView('display'); // Switch View
                    }}
                    className="p-3 cursor-pointer rounded-lg hover:bg-gray-100 focus:bg-gray-100 flex items-center gap-3 group"
                >
                    <Moon size={20} className="text-gray-600" />
                    <span className="font-medium text-gray-700 flex-1">Display</span>
                </DropdownMenuItem>
                ...
                >
                    ...
                    <span className="font-medium text-gray-700 flex-1">Report a problem</span>
                </DropdownMenuItem>
                ...
                >
                    {isSigningOut ? <Loader2 size={20} className="animate-spin text-red-600" /> : <LogOut size={20} className="text-red-600" />}
          ...
          {menuView === 'display' && (
             <div className="animate-in slide-in-from-right-2 duration-200">
                ...
                <div className="flex items-center gap-2 p-2 border-b border-gray-100 mb-2">
                    ...
                    <span className="font-semibold text-slate-700 text-sm">Switch appearance</span>
                </div>
                ...
                <div className="px-2 py-2">
                     <div className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 transition-colors">
                        <div className="flex items-center gap-3">
                             ...
                             {isDark ? <Moon size={18} className="text-gray-600"/> : <Sun size={18} className="text-orange-500"/>}
                        ...
                        <button 
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-200 ${
