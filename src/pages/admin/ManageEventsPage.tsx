...
export default function ManageEventsPage() {
  ...
  const handleSendNotification = async (message: string) => {
    ...
    try {
      await notifyAttendees(selectedEventId, selectedEventTitle, message, user.id);
  ...
  const handleDemote = async (registrationId: string) => {
    ...
    if (error) {
        ...
        notifyRegistrationUpdate(attendee.user_id, selectedEventId!, selectedEventTitle, "waitlist", user!.id);
    }
  };
  ...
  const handleRemove = async (registrationId: string) => {
    ...
    notifyRegistrationUpdate(attendeeToRemove.user_id, selectedEventId!, selectedEventTitle, "removed", user!.id);
  };
  ...
  const handleReject = async (registrationId: string) => {
    ...
    if (error) {
      ...
      notifyRegistrationUpdate(attendee.user_id, selectedEventId!, selectedEventTitle, "rejected", user!.id);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", `${selectedEventTitle || "guest-list"}.csv`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        {/* ✅ TOOLBAR: Moved Search & Create Here */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
            ...
            <div className="relative w-full sm:w-80">
                ...
                <Input
                    ...
                    className="pl-10 bg-white border-gray-200 shadow-sm focus:border-blue-500 transition-all h-10 rounded-xl"
                />
            </div>
            ...
            <Button 
                ...
                size="sm" 
                className="shrink-0 w-full sm:w-auto rounded-xl h-10 px-5 shadow-sm font-bold bg-gray-900 hover:bg-black"
            >
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-200 shadow-sm">
             {eventSearch ? (
                <>
                  ...
                  <h3 className="font-bold text-gray-900">No matching events</h3>
                  <p className="text-sm text-gray-500 mb-4">Try adjusting your search terms</p>
                  <Button variant="outline" onClick={() => setEventSearch("")}>Clear Search</Button>
                </>
             ) : (
                <>
                  <Calendar className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                  <h3 className="font-bold text-gray-900">No events found</h3>
                  <Button onClick={() => navigate("/admin/events/new-official")} className="mt-4">Create Event</Button>
                </>
             )}
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredEvents.map((event) => (
              ...
              >
                ...
                <div className="absolute top-3 right-3 z-20" onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40 z-30 bg-white">
                            ...
                            <DropdownMenuItem onClick={() => handleToggleHide(event.id, event.is_hidden)}>
                ...
                <span className="text-sm font-bold md:text-blue-600 text-gray-600 flex items-center gap-1 transition-all transform 
    ...
    >
      ...
      {/* Slide-over Detail Panel (Keep existing logic) */}
      {selectedEventId && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-white h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                            ...
                            <div key={a.id} className={`p-3 rounded-xl border flex items-center gap-3 shadow-sm transition-all ${a.status === 'waitlist' ? 'bg-amber-50/50 border-amber-100' : a.status === 'rejected' ? 'bg-red-50/30 border-red-100 opacity-60 grayscale-[0.5]' : 'bg-white border-gray-100'}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                        ...
                                        {a.pet && (<span className="flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-1 rounded-md text-[10px] font-bold border border-blue-100"><span className="uppercase text-blue-400">{a.pet.species}:</span>{a.pet.name}</span>)}
                                ...
                                <div onClick={(e) => e.stopPropagation()} className="flex items-center gap-2">
                                   {a.status === "waitlist" ? (
                                        <button onClick={() => handlePromote(a.id)} className="shrink-0 w-24 h-9 rounded-lg font-bold text-xs bg-amber-500 text-white hover:bg-amber-600 transition-all active:scale-95 shadow-sm">Approve</button>
                                    ) : a.status === "rejected" ? (
                                        <span className="shrink-0 w-24 text-center text-xs font-bold text-red-400">Declined</span>
                                    ) : (
                                        <button onClick={() => handleCheckIn(a.id, a.status)} className={`shrink-0 w-24 h-9 rounded-lg font-bold text-xs flex items-center justify-center gap-1 transition-all active:scale-95 ${a.status === "checked_in" ? "bg-green-100 text-green-700 border border-green-200" : "bg-black text-white hover:bg-gray-800"}`}>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-white z-110">
                                            {/* (Keep existing Guest action menu items here) */}
                                            {a.status === "approved" && (
                                                <>
                                                    <DropdownMenuItem onClick={() => handleDemote(a.id)} className="text-geo-600 focus:text-amber-700 focus:bg-amber-50"><XCircle className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Move to Waitlist</span></DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => handleReject(a.id)} className="text-amber-600 focus:text-amber-700 focus:bg-amber-50"><XCircle className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Reject</span></DropdownMenuItem>
                                                </>
                                            )}
                                            {a.status === "rejected" && (
                                                <><DropdownMenuItem onClick={() => handlePromote(a.id)}><Unlock className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Restore to Going</span></DropdownMenuItem><DropdownMenuItem onClick={() => handleDemote(a.id)}><Unlock className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Restore to Waitlist</span></DropdownMenuItem></>
                                            )}
                                            {a.status === "waitlist" && (
                                                <><DropdownMenuItem onClick={() => handleReject(a.id)} className="text-amber-600 focus:text-amber-700 focus:bg-amber-50"><XCircle className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Reject</span></DropdownMenuItem></>
                                            )}
                                            <DropdownMenuItem onClick={() => handleRemove(a.id)} className="text-red-600 focus:text-red-600 focus:bg-red-50"><Trash2 className="w-3.5 h-3.5 mr-2" /><span className="text-xs font-medium">Remove Completely</span></DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </div>
                            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-white"><Button className="w-full gap-2" variant="outline"
          onClick={handleExportCSV} disabled={attendees.length === 0}><Download className="w-4 h-4" /> Export Guest List (CSV)</Button></div>
          </div>
        </div>
      )}
