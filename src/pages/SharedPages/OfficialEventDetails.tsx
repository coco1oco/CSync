...
export default function OfficialEventDetailsPage() {
  ...
  // Safety check for waitlist calculation
  const isWaitlistMode =
    (event.max_attendees || 0) > 0 &&
    approvedCount >= (event.max_attendees || 0);

  const getDeadlineBadge = () => {
    if (isClosedManually)
      return {
        label: "Registration Closed",
        color: "bg-red-600 text-white shadow-red-500/30",
        icon: XCircle,
      };
    if (isDeadlinePassed)
      return {
        label: "Deadline Passed",
        color: "bg-gray-600 text-white shadow-gray-500/30",
        icon: Timer,
      };
    ...
    if (deadlineDate && !isNaN(deadlineDate.getTime())) {
      return {
        ...
        color: "bg-green-600 text-white shadow-green-500/30",
        icon: Clock,
      };
    }
    return null;
  ...
  const isHappeningNow = (() => {
    ...
    const [endH, endM] = (event.end_time || "23:59").split(":").map(Number);
    const endTotal = endH * 60 + endM;

    return currentMinutes >= startTotal && currentMinutes <= endTotal;
  })();

  const goingList = attendees.filter((a) =>
  ...
  const displayList = currentList.filter(
    (a) =>
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="relative h-[35vh] md:h-[45vh] w-full bg-gray-900">
        ...
        <div className="absolute top-6 left-6 z-20">
          <Button
            variant="secondary"
            size="sm"
            className="rounded-full bg-white/20 backdrop-blur-md hover:bg-white/40 text-white border-none shadow-xl"
            onClick={() => navigate(-1)}
          >
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 z-10">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              {deadlineBadge && (
                <div
                  className={`px-3 py-1.5 rounded-lg shadow-lg flex items-center gap-1.5 text-xs md:text-sm font-bold uppercase tracking-widest border border-white/10 ${deadlineBadge.color}`}
                >
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 mt-8">
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  displayList.map((att) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <img
                          src={att.user.avatar_url || "/default-avatar.png"}
                      ...
                      <div className="shrink-0">
                        {att.status === "checked_in" ? (
                          <span className="bg-blue-100 text-blue-700 p-1.5 rounded-lg flex items-center shadow-sm">
                            <UserCheck size={16} />
                          </span>
                        ) : att.status === "waitlist" ? (
          ...
          {/* RIGHT COLUMN: Registration (Sticky) */}
              ...
              <div className="bg-white rounded-3xl p-8 shadow-xl shadow-blue-900/5 border border-gray-100">
                ...
                <div className="mb-6">
                  {isClosed && !isRegistered ? (
                    ...
                    <p className="text-gray-500 font-medium">
                      {isRegistered
                        ? "Successfully registered! We can't wait to see you."
                        : "Claim your spot for this exclusive campus event."}
                    </p>
                  )}
                </div>
                  ...
                  <div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          (approvedCount / (event.max_attendees || 1)) * 100
                        )}%`,
                      }}
                    />
                  </div>
