...
export default function SettingsPage() {
  ...
  const handleSignOut = async () => {
    ...
    try { await signOut(); navigate("/SignIn"); } catch (error) { setIsSigningOut(false); }
  };
      ...
      <div className="w-full">
        ...
        <div className="mb-6 flex items-center gap-3 md:pl-1">
            <button 
                ...
                className={`p-2 rounded-full hover:bg-gray-200 text-gray-700 md:hidden ${showMobileDetail ? 'block' : 'hidden'}`}
            >
                <ArrowLeft size={24} />
            </button>
            <div className="flex items-center gap-2">
                <Settings className={`w-8 h-8 text-gray-400 hidden md:block ${showMobileDetail ? 'hidden' : ''}`} />
                <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
                    {showMobileDetail ? menuItems.find(i => i.id === activeTab)?.label : "Settings"}
                </h1>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start w-full">
            ...
            {/* --- Sidebar Menu --- */}
            <nav className={`w-full md:w-64 shrink-0 md:sticky md:top-6 transition-all ${showMobileDetail ? 'hidden md:block' : 'block'}`}>
                
                {/* Mobile Profile Card */}
                <div className="md:hidden mb-4">
                    <Link to="/ProfilePage" className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 active:scale-[0.98] transition-transform">
                        <img 
                ...
                <div className="bg-transparent md:space-y-1">
                    {menuItems.map((item) => {
                        ...
                        const isMobileOnly = item.id === "display" || item.id === "report";
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-3">
                                    <div className={`p-1.5 rounded-md transition-colors ${isActive ? 'bg-blue-50 text-blue-600' : 'bg-transparent text-gray-400 group-hover:text-gray-600'}`}>
                                        <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} />
                                    </div>
                                    <span className={`text-base md:text-sm ${isActive ? 'font-semibold' : ''}`}>{item.label}</span>
                                </div>
            ...
            {/* --- Main Content Area --- */}
            <main className={`flex-1 min-w-0 bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8 pb-28 md:pb-8 w-full ${showMobileDetail ? 'block' : 'hidden md:block'}`}>
                {renderContent()}
            </main>
